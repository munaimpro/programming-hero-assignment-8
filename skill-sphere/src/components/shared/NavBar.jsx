        ...
        <motion.div initial={{ y: -100 }} animate={{ y: 0 }} transition={{ duration: 0.4 }} className="navbar bg-base-100 shadow-sm fixed top-0 left-0 w-full z-50">
            <div className="container mx-auto flex">
                <div className="navbar-start">
                    ...
                    <Link className="text-2xl lg:text-3xl font-bold text-[#082a5e]" href={'/'}>SkillSphere</Link>
                </div>
                        ...
                        isPending ? (
                            ...
                            <>
                                <Image className="rounded-full" src={user?.image || userAvater} alt={user?.name || "User"} width={40} height={40} />
                                    ...
                                    <Link onClick={async () => await authClient.signOut()} className="btn btn-primary shadow-none" href={'/login'}>
                                        Logout
                                    </Link>
                            </>
                        ): (
